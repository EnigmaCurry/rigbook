        ...
        {#each sortedSpots as spot, i (spot.callsign + spot.frequency + spot.mode)}
          ...
          <tr class:worked={isWorkedToday(spot)}
            ...
            {#each spotColumns as col (col.key)}
              ...
              {:else if col.key === "location"}<td class="location">{#if isPotaActivator(spot)}{@const pota = getPotaSpot(spot)}<!-- svelte-ignore a11y-click-events-have-key-events --><!-- svelte-ignore a11y-no-static-element-interactions --><span class="pota-loc clickable" on:click|stopPropagation={() => openParkModal(spotHomeLabel(spot))}>{spotHomeLabel(spot)}</span>{#if pota?.name}<span class="pota-name">{pota.name}</span>{/if}{:else if spot.qrz_grid && !spot.qrz_grid_approx}{spot.qrz_grid} <span class="loc-detail">{locationStr(spot)}</span>{:else if spot.qrz_state && spot.country}{spot.qrz_state}, {spot.country}{:else if spot.country}{spot.country}{:else if (spot._qrz_status || spot.qrz_status) === "not_found"}<span class="fetch-hint">(No QRZ record)</span>{:else if (spot._qrz_status || spot.qrz_status) === "no_location"}<span class="fetch-hint">(No QRZ location)</span>{:else if !qrzConfigured}<span class="fetch-hint">(Configure QRZ account)</span>{:else if qrz.skipped}<span class="fetch-hint">(filter more to fetch)</span>{:else if qrz.pending > 0}<span class="fetch-hint">(fetching... {qrz.pending} left)</span>{/if}</td>
